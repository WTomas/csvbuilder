  ...
  describe("Padded concatenation", () => {
    ...
    beforeAll(() => {
      builder = new CSVBuilder<Template>()
        ...
        .concat(
          new CSVBuilder<Template>()
            .createColumn("id", [3, 4])
            .createColumn("colour", ["green", "blue"])
        )
        .setBuilderOptions({ emptyValue: "" });

      expectedCsv = loadCSVFixture("concat/basic");
    });

    it("Should produce a padded CSV with the right dimensions", () => {
      const [csv, dims] = builder.getStringAndDimensions();
      expect(csv).toBe(expectedCsv);
      expect(dims).toMatchObject({ nRows: 5, nCols: 3 });
    });
  });
});
